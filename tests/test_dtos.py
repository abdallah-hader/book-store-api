...
def test_signup_lowercases_username_and_email():
    user = UserCreateRequest(username="abdallah", email="abdallah@gmail.com", password="a123456789")
    assert user.username == "abdallah"
    assert user.email == "abdallah@gmail.com"

def test_signup_ignores_a_role_the_client_tries_to_set():
    user = UserCreateRequest(
        username="abdullah", email="abd@example.com", password="abdalla99", role="admin"
    )
